...
async function runIntegrationTest() {
    ...
    console.log('  -> Strategy:', plannerResult.plan.globalStrategy); // Expect LAYERED_DYNAMIC (because Visualizer)
    ...
    console.log('  -> Pipeline State:', pipelineBuilder.state);
    console.log('  -> Total Active Nodes:', pipelinePlan.nodes.filter(n => n.status !== 'SKIPPED').length);
    console.log('  -> Total Stages:', pipelinePlan.stages.length);
    console.log('  -> Pipeline Version:', pipelinePlan.pipelineVersion);
    console.log('  -> Sample Metadata:', pipelinePlan.metadata.estimatedFilterCount, 'filters');
