export default class Audio {
  ...
  initBuffer() {
    ...
    if (this.pitch !== undefined && this.pitch !== 0) {
      ...
      } else if (this.source.playbackRate) {
        this.source.playbackRate.value = Math.pow(2, this.pitch / 12);
      }
    }
  ...
  setPitch(semitones = 0) {
    ...
    if (this.source) {
      if (this.source.detune) {
        this.source.detune.setValueAtTime(semitones * 100, this.audioContext.currentTime);
      } else if (this.source.playbackRate) {
        this.source.playbackRate.setValueAtTime(Math.pow(2, semitones / 12), this.audioContext.currentTime);
      }
    }
  }
