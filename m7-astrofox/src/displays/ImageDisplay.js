...
export default class ImageDisplay extends WebGLDisplay {
  ...
  update(properties) {
    ...
    if (changed) {
      ...
      if (image) {
        ...
        const size = this.scene && this.scene.getSize ? this.scene.getSize() : { width: 1920, height: 1080 };
        const w = size.width || 1920;
        const h = size.height || 1080;

        this.pass = new ImagePass(texture, { width: w, height: h });

        this.pass.camera.aspect = w / h;
        this.pass.camera.updateProjectionMatrix();
      }
      if (this.pass) {
        const naturalW = this.image ? (this.image.naturalWidth || this.image.videoWidth || this.image.width || 1920) : 1920;
        const naturalH = this.image ? (this.image.naturalHeight || this.image.videoHeight || this.image.height || 1080) : 1080;
        ...
        if (width) {
          this.pass.mesh.scale.x = width / naturalW;
        }
        if (height) {
          this.pass.mesh.scale.y = height / naturalH;
        }
        if (opacity) {
  ...
  addToScene({ getSize }) {
    ...
    } else if (typeof this.properties.src === 'string' && this.properties.src !== BLANK_IMAGE) {
