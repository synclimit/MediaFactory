...
const targetStr = `  // Global drag handler for image to bypass DOM bubbling issues
  useEffect(() => {
...
const replaceStr = `  // Global drag handler for image to bypass DOM bubbling issues
  useEffect(() => {
    if (!isDraggingImage) return;
...
if (content.includes(targetStr)) {
  ...
  console.log('Restored M5NewsCreator.jsx successfully.');
} else {
  console.error('Could not find target string in M5NewsCreator.jsx');
}
