...
export default function WorkspacePicker({ activeWorkspace, onWorkspaceSelected, onNewWorkspace }) {
    ...
    const currentActiveName = activeWorkspace || localStorage.getItem('mf_active_workspace') || '';

    const formatLastOpened = (val) => {
        if (!val) return 'NEVER';
        try {
        ...
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center font-sans overflow-hidden bg-[#0a0a0c]">
            ...
            <div className="max-w-6xl w-full p-12 relative z-10">
                ...
                <div className="flex flex-wrap justify-center gap-6">
                    {workspaces.map((ws) => {
                        const isWsActive = ws.isActive || Boolean(currentActiveName && (ws.name.toLowerCase() === currentActiveName.toLowerCase() || (ws.folderName && ws.folderName.toLowerCase() === currentActiveName.toLowerCase())));
                            ...
                            >
                                ...
                                <div className="flex items-start gap-4 mb-4 relative z-10">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <h3 className="text-[20px] font-black text-white tracking-widest uppercase mb-1 drop-shadow-[0_0_8px_rgba(255,255,255,0.2)] truncate">{ws.name}</h3>
                                        <p className="text-[11px] text-gray-400 font-mono mb-2">LAST OPENED: {formatLastOpened(ws.lastOpened)}</p>
                                        
                                        {/* Real-time Active Indicator */}
                                        {isWsActive ? (
                                            <div className="flex items-center gap-1.5 bg-emerald-950/40 border border-emerald-500/40 px-2 py-0.5 rounded w-fit">
                                                <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(16,185,129,1)] animate-pulse"></div>
                                                <span className="text-[10px] font-bold text-emerald-400 tracking-wider uppercase font-['Rajdhani']">ACTIVE</span>
                                            </div>
                                        ) : (
                                            <div className="flex items-center gap-1.5 bg-gray-900/60 border border-[#2d3142] px-2 py-0.5 rounded w-fit">
                                                <div className="w-1.5 h-1.5 rounded-full bg-gray-500"></div>
                                                <span className="text-[10px] font-bold text-gray-400 tracking-wider uppercase font-['Rajdhani']">STANDBY</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                {selected === ws.name && (
                                    <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black via-black/90 to-transparent flex justify-center gap-2 animate-in fade-in slide-in-from-bottom-2 duration-250 z-20">
                                        <button 
                                            ...
                                            className="w-10 h-10 flex items-center justify-center bg-[#1a1c23] hover:bg-red-900/30 border border-[#333] hover:border-red-500/50 rounded-lg text-gray-400 hover:text-red-400 transition-all shadow-[0_2px_10px_rgba(0,0,0,0.5)] cursor-pointer"
                                        ...
                                        <button 
                                            ...
                                            className="w-10 h-10 flex items-center justify-center bg-[#1a1c23] hover:bg-[#2a2c33] border border-[#333] hover:border-orange-500/50 rounded-lg text-gray-400 hover:text-white transition-all shadow-[0_2px_10px_rgba(0,0,0,0.5)] cursor-pointer"
                                            title="Sync & Refresh Stats"
                                        >
                                            <FolderSync size={16} />
                                        </button>
                                        <button 
