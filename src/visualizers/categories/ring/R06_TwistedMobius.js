...
export const metadata = {
    id: 'ring-mobius',
    name: 'TwistedMobius',
    displayName: 'Twisted Möbius Ring',
    description: 'A 3D twisted ring mapping frequency data to its folds',
    category: 'Ring',
...
export function render(context) {
    ...
    for (let i = 0; i <= segments; i++) {
        ...
        // Parametric Mobius-like fold
        const fold = Math.sin(t * 3 + (elapsedTime || 0)) * (50 + pulse);
        
        const x = cx + Math.cos(t) * (r + fold);
        const y = cy + Math.sin(t) * (r - fold);
