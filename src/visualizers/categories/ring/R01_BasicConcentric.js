...
export const metadata = {
    id: 'ring-basic',
    name: 'BasicConcentric',
    displayName: 'Basic Concentric Rings',
    description: 'Standard audio-reactive concentric rings',
    category: 'Ring',
...
export function render(context) {
    ...
    const baseR = radius || 50;
    
    ctx.strokeStyle = color || '#00ffcc';
    ctx.lineWidth = thickness || 2;
    
    // Draw rings for each frequency band
    const activeBands = Math.min(10, barCount); // Don't draw 64 rings
    const ringSpacing = (viewport.height / 2.5) / activeBands;
    
    for (let i = 0; i < activeBands; i++) {
        // Average a chunk of frequencies for this ring
        const chunk = Math.floor(rawData.length / activeBands);
        let sum = 0;
        for (let j=0; j<chunk; j++) sum += (state.smoothedData[i*chunk + j] || 0);
        const avg = sum / chunk;
        
        const h = (avg / 255) * (ringSpacing) * (gain || 1.0);
        const currentR = baseR + (i * ringSpacing) + h;
