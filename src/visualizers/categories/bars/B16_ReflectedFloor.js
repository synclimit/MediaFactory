...
export const metadata = {
    ...
    description: 'Bars with a mirrored floor reflection effect',
    category: 'Bars',
...
export function render(context) {
    ...
    const floorY = viewport.height * 0.75;
    
    for (let i = 0; i < barCount; i++) {
        const val = state.smoothedData[i] || 0;
        const h = (val / 255) * (floorY) * (gain || 1.0);
        ...
        // Reflection
        ctx.fillStyle = (color || '#00ffcc') + '44'; // 25% opacity
        ctx.fillRect(x, floorY, barWidth, h * 0.5);
    }
}
