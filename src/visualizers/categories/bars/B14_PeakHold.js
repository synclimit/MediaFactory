...
export const metadata = {
    ...
    description: 'Bars with little blocks that float at the peak and fall slowly',
    category: 'Bars',
...
export function render(context) {
    ...
    for (let i = 0; i < barCount; i++) {
        ...
        if (h > state.peaks[i]) {
            ...
            state.peaks[i] -= 2; // Fall speed
            if (state.peaks[i] < 0) state.peaks[i] = 0;
        }
        
        const x = startX + (i * step);
        const y = viewport.height - h;
        const peakY = viewport.height - state.peaks[i];
        ...
        ctx.fillRect(x, peakY - barWidth, barWidth, barWidth);
        ctx.fillStyle = color || '#00ffcc';
    }
}
