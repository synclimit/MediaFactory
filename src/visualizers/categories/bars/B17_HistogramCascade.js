/**
 * B17_HistogramCascade.js
 * Histogram Cascade Bars
 */
import { MathUtils } from '../../utils/MathUtils';

export const metadata = {
    id: 'bars-histogram',
    name: 'HistogramCascade',
    displayName: 'Histogram Cascade Bars',
    description: 'Layered histogram effect fading into the background',
    category: 'Bars',
...
export function render(context) {
    ...
    for (let row = state.history.length - 1; row >= 0; row--) {
        ...
        for (let i = 0; i < barCount; i++) {
            ...
            const x = startX + (i * step) + (row * 5); // offset cascade
            const y = viewport.height - h - (row * 5);
