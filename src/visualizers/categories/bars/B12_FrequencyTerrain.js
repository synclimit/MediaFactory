/**
 * B12_FrequencyTerrain.js
 * Frequency Terrain Bars
 */
import { MathUtils } from '../../utils/MathUtils';

export const metadata = {
    id: 'bars-frequency-terrain',
    name: 'FrequencyTerrain',
    displayName: 'Frequency Terrain Bars',
    description: 'Connected bars forming a terrain or mountain range',
    category: 'Bars',
...
export function render(context) {
    ...
    ctx.beginPath();
    ctx.moveTo(startX, viewport.height);
    
    for (let i = 0; i < barCount; i++) {
        const val = state.smoothedData[i] || 0;
        const h = (val / 255) * viewport.height * (gain || 1.0);
        const x = startX + (i * step) + (barWidth / 2);
        const y = viewport.height - h;
        
        ctx.lineTo(x, y);
    }
    
    ctx.lineTo(startX + totalWidth, viewport.height);
    ctx.closePath();
    ctx.fill();
}
