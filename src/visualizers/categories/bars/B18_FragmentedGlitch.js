...
export const metadata = {
    id: 'bars-fragmented',
    name: 'FragmentedGlitch',
    displayName: 'Fragmented Glitch Bars',
    description: 'Glitchy fragmented bars that slice horizontally',
    category: 'Bars',
...
export function render(context) {
    ...
    for (let i = 0; i < barCount; i++) {
        ...
        if (h > 0) {
            // Main solid body
            ctx.fillRect(x, y + 20, barWidth, h - 20);
            
            // Glitch fragments
            const glitchOffset1 = (Math.random() - 0.5) * 10;
            ctx.fillRect(x + glitchOffset1, y + 5, barWidth, 10);
            
            const glitchOffset2 = (Math.random() - 0.5) * 5;
            ctx.fillRect(x + glitchOffset2, y - 10, barWidth, 10);
        }
    }
}
